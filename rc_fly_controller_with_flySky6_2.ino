...
// =============== OFFSET KIRITING ===============
float RateCalibrationRoll = 1.9649;
float RateCalibrationPitch = -0.1925;
float RateCalibrationYaw = -1.5919;

float AccXCalibration = 0.0728;
float AccYCalibration = 0.0656;
float AccZCalibration = -0.0397;
// =================================================
...
void loop() {
  ...
  // Stabilization gains - OSHIRILDI!
  float k_angle = 20.0;    // Angle correction (angle-based)
  float k_rate = 1.2;      // Rate correction (gyro-based) - YANGI!

  // Calculate corrections - ikkala signal ishlatiladi
  float rollCorr  = (compRoll * k_angle) + (RateRoll * k_rate);
  float pitchCorr = (compPitch * k_angle) + (RatePitch * k_rate);
  ...
  servoPitch.writeMicroseconds(map(pitchOut, 1000, 2000, 2000, 1000));
  servoYaw.writeMicroseconds(yawOut);

  // Serial output for monitoring
  Serial.print(compRoll, 2);
  Serial.print("\t");
  Serial.print(compPitch, 2);
  Serial.print("\t");
